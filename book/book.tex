...
\section{Understanding processes and fork()}

You will often be confronted with problems related to determining the number
of process resulting to a fork system call in different program. Here, we
are going to go over a series of examples to master this common question.
Before diving into this, let’s refresh ourselves about the definition of a
process. 
Processes are instances of programs on a computer. This simply means that,
when a program is executed, the kernel creates a new process along with a
PCB (Process Control Block) which keeps track of all the information
allowing the execution of the process. A PCB comprises at a minimum of (a)
Process ID, (b) Process Status, (c) Open File descriptors and (d) CPU
registers.

Remember the following key points when trying to solve the following problems:

\begin{itemize}
    \item Processes are created using the fork system call, which returns
    the PID (process ID) of the child process.
    
    \item Child processes created using fork receive an entire copy of the
    parent’s current memory image including the same current values to every
    variable.
    ...
    \item The new/child process will only start after the exact line where
    it was created by fork.
    
    \item The only difference between a parent and a child process will be
    their PID which effectively distinguishes their respective return values
    from fork – The child is returned a 0, meanwhile the parent is returned
    a positive integer which is identical to the child’s PID.
    This is used in an if else statement to separate the code of the parent
    from that of that of the child,
    
    
    
\end{itemize}

The following example illustrates how fork (basically) works:
...
Output:
I am the parent! My pid is 3621
I am the child! My pid is 3622

\end{verbatim}

Now, let’s look at some complicated examples of fork with the following
scenarios:

\begin{enumerate}
    \item How many hi’s are printed to the screen?
...
Answer = 8
 ...
 \includegraphics[width=5.99in,height=3.17in]{fork.jpg}
 
This question is equivalent to asking you, how many processes including the
parent result from this program. As shown by the diagram, the number of hi’s
or processes resulting from this program is 8. An effective way of
calculating the number of processes created by a fork() under a “for loop”
running n times is by calculating the value $2^n$-1. This gives
the total number of children processes minus the parent process. 


\item Now test yourself by figuring out the output in the following program
based off the previous explanation.
...
\end{verbatim}
    ...
    \item How many processes including the parent process are created in this next program.
...
\end{verbatim}
    ...
    \item How many processes including the parent process are created in the
    following program?
...
Notice that the operation fork()==fork() is false as a result of each fork
producing different child processes, with different PIDs. Hence, the return
values of the forks are different. So 2 children are created in the if
condition, and those 2 children (instead of the parent) execute in the if
statement, thereby forking again. We end up with 1 parent + 2 children  + 2
grand children = 5 processes.  

\end{enumerate}

When presented with these types of problems, it will be helpful to trace
every fork on a sheet of paper keeping track of every parent to child
relationship to help you visualize it more easily and quicker. 
...
\section{Threads and CPU}

Understanding how the CPU works with threads will give a clearer picture
about what threads are and how they work. This section will focus on how
threads relate to CPU execution and how they affect the way that your
system performs.

You may have heard, “thread” and “thread of execution” being used in
different scenarios. But in reality “thread” is just used to simplify
the idea of “thread of execution”. So, they may be used interchangeably.
When you run a command, the CPU begins the fetch, decode, and execution
process to accomplish the command. The thread is the sequence of
instructions that describe what the computer must do in order to achieve
that command!

Let’s take a closer look at the comparison between threads and the cores
in your CPU. Each of the cores in your CPU can be pictured as {\bf
workers} who do whatever task their boss, the Operating System, assigns
them to do. On the other hand, threads are the sequences of instructions
given to the cores, so we can depict threads as {\bf conveyor belts} of
products being sent to the workers. To understand the general purpose o
f threads and cores, it’s helpful to know the difference between a
physical core and a logical core. A physical core is simply the actual
core (hardware) that exist on the computer, meanwhile logical cores can
be depicted as the threads of the computer. For instance, if your CPU
processor has 4 cores and 4 threads (1 Thread/Core), you will have 4
logical cores, and 4 physical cores. Assuming you had a processor with 4
cores and 8 threads (2 Threads/core), you will then have 8 logical cores
and 4 physical cores. So logical cores are effectively the number of
pathways for logic/information to be processed through your computer.

Now what happens when your CPU has more threads than cores, like in the
case of 8 threads running on your 4 core CPU? Well your cores will each
be given 2 threads of commands to execute. With the worker/conveyor-belt
analogy, the workers are given 2 lines of conveyor belt products.
Usually, during normal CPU operations, you won’t see continuous
operations on a single thread. There are often small gaps between the
instructions on a thread that causes downtime, because the computer
could be waiting on resources like disk IO, network IO or user input.
This is where the advantage of having that second thread arises. You
might have only one core but having this second thread generates more
efficiency as the core can switch to actively performing instructions
available on the 2nd thread while waiting for the computer to catch up
on the 1st thread’s resources needs. This is analogous to the worker
rotating between the 2 conveyor belts when one of them runs out of products
in order to generate an efficient throughput.

This scenario provides us a clear juxtaposition of Parallel Operation vs
Concurrent Operations. The core to core operation is an example of
Parallelism since all 4 cores on your computer work simultaneously and
at the same time. On the other hand, the cores themselves can’t
work/execute on multiple threads at the same time, so they must switch
between single threads at a time. This independent execution of threads
by switching between them is called Concurrency - although the switching
between the threads happens quickly enough that the threads might appear
to run simultaneously.
We notice that, multiple threads per core can help in getting full
efficiency out of each core by filling in the gaps of downtime, thereby
enhancing performance. 
However, “more threads” isn't always necessarily faster. it's difficult
to assert that without knowing more about the nature of your threads.
Things to take into consideration when diagnosing system performance
include:

Is the thread/process
\begin{itemize}

    \item CPU bound (needs lots of CPU resources)
    \item Memory bound (needs lots of RAM resources)
    \item I/O bound (requires Network and/or hard drive resources)
    
\end{itemize}

Each of these three resources are finite, and each of them could limit
the performance of a system. You need to look at which your thread is
consuming. The more threads you have which fall in category 3 or 2, the
more you take time to switch from one thread to another which is pure
overhead because your system does NO work while switching. CPU-bound
code, on the other hand, is inherently bound by the rate at which the
CPU can execute instructions. So, when your code runs in parallel across
4 threads on your various cores, you’re utilizing all the power of your
4-core CPU without any overhead! 

Another risk of multi-threading is that you need to be extra careful
about the amount of data shared between your threads and hence, the
amount of locking your code needs to do. If your threads are locked too
often, your code may run as slow as a single-threaded code or even enter
an infinite loop as a result of an induced race-condition such as a
deadlock.  A lock (or mutex) is a synchronization mechanism for
enforcing limits on access to a specific region of your code. Assume you
have two threads, and they are both trying to get locks/mutexes P and Q.
If the first thread already has P, and the second thread already has Q,
then the threads will be deadlocked. This is because the first thread
can’t get mutex Q and the second thread can’t get mutex P. As a result,
your 2 threads come to a standstill. Locks and Deadlock (prevention) are
further discussed in depth by the end of this chapter.


\section{Threads vs Processes}

Processes can be used to do several things at the same time by; (a) creating
new (child) processes with the fork() system call as seen under chapter 8,
(b) connecting multiple client processes to a server over a network socket.
Each time a client will connect, the server assigns the client a new task to
perform.

Although processes can be used to multitask on a computer, they are not
always the best option. We require something that can start a separate task
quickly enough, can share all our current data and won’t need a lot of code
to build. Threads provide these features!

\begin{itemize}

    \item Threads are faster to create than processes. Some machines take a
    little more time to bring processes to life. If you just had to perform
    a simple task that required 0.01 second, creating a process every time
    for this won’t be very efficient.
    
    \item Threads share data more easily than processes. When a new process
    is created, it automatically gains a separate copy of the parent
    process. There are no links between the parent and child processes. If
    the new process needed to send data back to the parent process, then it
    will require something like a pipe to connect them together. On the
    other hand, newly created threads all have their own stack, and
    registers but share the same heap and static memory with their parents.
    Hence, they can share data more easily and rapidly through the heap.
    
    \item Context switch between threads of the same process is less
    expensive than context switching between processes.
...
These features, of course, don’t make threads better than processes in every
situation.  Processes are typically independent of each other, while threads
exist as the subset of a process. As a result, threads are more vulnerable
to problems caused by other threads in the same process. This leads to one
of the main advantage of processes over threads which is {\bf Fault
Tolerance}: When processes crash, you can feel relatively safe knowing that
you can just restart them (even across network host). In contrast, if a
thread crashed, it may crash the entire process, which may bring your entire
application down.
...
\chapter{Top 7 Programming Concepts/Practices Needed for OS}

Meanwhile this course is designed to expose you to the theory and design of
operating systems, a significant part of the course also deals with
implementing (simplified) parts of the operating system. The implied
assumption is that most of the students taking this course are well versed
with programming in a “lower” level language like C++. However, a lot of
students come into this course without full confidence in C, which is the
most popular language that comes up in systems programming! So, I think it
would be a big benefit to students to refresh or learn about a few
non-trivial programming concepts and practices in C language before diving
into this course. This chapter will go over the top 7 programming concepts
in C that will come in very handy for this class. 

\section{Memory of C Programs}

Although C is considered as a "high-level"  language, it comes at the
"low-level"  end of the language spectrum because of the following main 

\begin{itemize}
	\item C uses pointers which allow direct access and manipulation of
	memory contents.

	\item C provides a good set of bit manipulation operators, allowing
	bitwise operations

	\item C has the possibility to dip in and out of assembly language
	depending on hardware architecture and compiler support for inline
	assembly language.

	\item  You may write non-portable C code that makes assumptions about
	the compiler and the underlying hardware architecture which may take
	advantage of hardware specific behaviours. 

\end{itemize}

Because of this reason, it’s important to understand how the memory of C
programs are layout and maintained to avoid committing potentially harmful
actions on your computer. 

When you execute your program, the kernel (program the core of a computer’s
operating system) creates a new process and will initialize the following
segments into the program’s memory: (Note the specific layout/order on a
standard x86 computer) 

\begin{itemize}
    \item Stack
    \item  ↑↓ (growth direction in free memory)
	\item Heap
	\item Data  
	\item Text  
\end{itemize}{}

\textbf{The text} (or constant) segment contains the program instructions (code) copied from the program executable. This segment is “read-only” memory. It may only be read by any process or subroutines. 

\textbf{The data} (or global) segment contains the global variables in your program. This region of memory is “read-write” - It is accessible by every part of the program. ex. Every functions and threads of execution can access this section of memory. Local variables are stored in a different memory segment from global variables because many local variables might get many instances/copies of them themselves if you write a function that calls itself. Moreover, their scopes and visibility are different (Read more under “Variable…”).

\textbf{The heap} (and sometimes the stack) is the memory segment for dynamic memory allocation. This includes pointer variables and pointer functions, which are allocated at run time. (See next section on pointers)  
The stack (short for call stack) is the memory segment where most functions calls along with their automatic variables are stored. Automatic variables are local variables which are allocated and deallocated when the program enters and leaves the variable’s scope (see section below under “Variables…”). 

\textbf{The stack} contains a “stack pointer” which tracks the top of the stack. Each time a function is called, it gets pushed on top the stack in a structure called “stack frame” with a return address and other information. The newly called function then allocates space on the stack memory for its automatic variables. Every time a recursive function calls itself, a new stack frame is used so that one’s set of variables doesn’t interfere with those of another instance of the function. This is how recursion is able to work.

As resources get allocated on the heap and the stack, they grow towards each other (stacks grow upward, and heap grows downward). If either of the heap or stack grows beyond the point of touching the other, the memory assigned to the program by the computer gets full and the program crashes as a result of a segmentation fault (an attempt to access a restricted area of memory). 
Such growth of the heap memory could be triggered by continuous allocation of memory without freeing that memory space after using it or simply by dynamically allocating large number of variables. This is called \textbf{Heap Overflow}. E.g
...
int main(){
	for (int i=0; i<10000000; i++) { 
	//Dynamically allocating large number variables
...
Meanwhile an excessive growth of the stack could be as a result of infinite recursive calls or by declaring large number of local variables (ex: int grid[100000][100000] ). This is known as \textbf{Stack Overflow}.  

\section{Segmentation Faults}
Segmentation Faults (aka segfault) or access violation are one of the most common error conditions that will crash your programs, so mastering all their potential sources is a great asset to acquire. In addition to the stack overflow and heap overflow examples described above, here is a list of common mistakes that lead to segmentation faults:
\begin{itemize}
	\item Writing to read-only memory
	\item Accessing an array out of bounds
	\item Dereferencing NULL pointers
	\item Dereferencing freed memory
	\item Dereferencing uninitialized pointers
	\item Incorrect use of the "\&" (address of) and "*" (dereferencing)
	operators
	\item Improper formatting specifiers in printf and scanf statements
\end{itemize}

\section{Pointers and Dynamic Memory}

If you really want to kick butts with C, you need to understand how it handles the memory of its programs. Pointers are the hotline between your program and its memory! After understanding how C program memory works, it’s a perfect timing to master how you can address it from your code. After all, pointers are key to becoming a kick-ass C programmer. 

A \textbf{pointer} is basically the \textbf{address} of a piece of data in memory. Pointers are used for the following main reasons:

    \begin{itemize}
        \item Passing around address of data (variables or functions) to functions instead of passing around a whole new copy for the intended data – By default, C passes its arguments by value. Pointers are used to pass by reference,  
    	\item When you want more than one piece of code to work on the same piece of data instead of a separate copy of the data. 
    \end{itemize}{}
...
The computer allocates a memory location for variable x, say 0x16B, on the stack memory and memory location 0x6E5 in the global memory. \textbf{To retrieve these addresses, C uses the \& operator}:

\begin{verbatim}
    printf("x is stored at %p\n", &x);	//passing the address/pointer of x
                                        //to printf(use %p)
\end{verbatim}

Now that you have the address of something, how do you store it? 

\begin{verbatim}
    int *address_of_x = &x; 		// this is called referencing

\end{verbatim}

We declare a pointer by preceding the variable name with the * operator in the variable declaration. Next, \textbf{to read the data stored in this memory address/pointer, we use the * operator} again like this:

\begin{verbatim}
    int value_stored = *address_of_x;	// this is called dereferencing
    
\end{verbatim}{}
	
To change the contents of an address, dereference the pointer then assign the value :
\begin{verbatim}
*address_of_x = 44;     // you must dereference pointer before assigning its
                        // value!
\end{verbatim}{}
...
Pointers have a fixed size irrespective of what data type they are pointing to. On a 32-bit operating system, a pointer takes 4 bytes of memory while on a 64-bit operating system, a pointer takes 8 bytes of memory. 

Why then do pointers have types?

Although pointers are just addresses, they need to have types corresponding to the contents of the addresses that they point to. This is so that the compiler knows how to adjust the pointer arithmetic.
Suppose you had an int pointer int. The size of an int is usually bytes, so if you added  1 to an int pointer, the compiled code will rather add 4 to the memory address so you can be pointing to the next integer in memory.
Int numbers[] = {1, 2, 3};
printf("numbers is at %p\n", numbers);
printf("numbers + 1 is at %p\n", numbers + 1);
The 2 memory addresses printed out from this code will be 4 bytes apart instead of 1.

\section{Variables and the Static Keyword}

Two important properties of variables in C are their persistence and visibility/scope.  Static and global variables are persistent since they hold their values for the duration of the execution of the program. In contrast, automatic/local variables become invalid when the program execution exits the scope where they are declared.

Meanwhile \textbf{global} variables have the program scope meaning they can be seen anywhere in the entire program provided a declaration is visible, \textbf{Automatic} variables can only have function and block scope (area within if and loop statements).  

\textbf{Static variables} can be used to control the scope of these variables within the .c file, a function, variable or even a block scope.
Good programming practices usually suggest that variables be given the smallest scope to do the job. This prevents accidental access via things like misspelling the variable name. It also limits name clashes for commonly used variable names and makes the logic easier to follow. When you start to write large programs, you need to be mindful about using too many global variables as they can create buggy code.
...
\end{verbatim}{}
 ...
 It uses the global variable count since it is in the global scope. This is very bad as any function can change its value and you would run into issues keeping track of which function in a large program. Luckily, you have the static keyword to fix this problem thereby limiting the scope of this variable to the counter function. 
...
The static keyword will store the variable inside the global area of memory, but the compiler throws an error if any different function from counter tries to access the variable. This also allows the count variable to be usable throughout the duration of the program execution since it is now a static variable.

\begin{itemize}
    \item Note: Static variables are initialized only once! 
    \item So, the value of count in the above snippet isn’t reset to 0 during every function call to counter. 
\end{itemize}{}

Static functions and static variables declared outside of functions are used to imply that “only code in this .c file may access this”. Example: 

\begin{verbatim}
static int days = 365;
static void update_score(int x) 
{ 
…
}    
...
A struct is a C data type that allows you to combine other basic data types into them. Struct stands for \textbf{structured data type}. In comparison to an array variable which is simply a pointer to the array, a struct variable is literally the name for the struct itself. This is because struct items are not perfectly contiguous in memory as a result of padding.  
...
A struct identifier(variable name) for this struct will have the data type struct fish var\_name. and will be declared as:

\begin{verbatim}
    struct fish bubbles = {“Bubbles”, 4, “Tilapia”, 1};
    
\end{verbatim}{}

Notice that in the above example, the fields of the struct need to be declared in the same order as in the struct definition. To by pass this, C allows struct declaration using \textbf{designated initializers} which are used as follows:

\begin{verbatim}
Struct fish bubbles = { .age = 1, 
                        .teeth = 4, 
                        .name = “Bubbles”,  
                        .specie = “Tilapia” };
...
C allows you to give your struct a proper name by aliasing it using the typedef keyword as follows:
...
fish bubbles = {“Bubbles”, 4, “Tilapia”, a};

You can set or update the value of a struct using the dot notation:

    Bubbles.teeth = 5;
    
\end{verbatim}{}

In C, function parameters are passed by value. So you need to make sure
to pass your struct by reference if you want to make a change to any of
the struct items. To do that you need to create a pointer to your struct,
or pass it using the \& operator in the function call. 

\begin{verbatim}

void happy_birthday(fish *f)
{
   f->age++; // Use -> to access fields of a [struct] pointer instead of the dot.
   printf("Happy Birthday %s! You are now %i years old!\n",f->name, f->age);
}
...
happy_birthday(&myfish);
\end{verbatim}{}

\begin{itemize}
    \item Note: pointer->field == (*pointer).field != *pointer.field
\end{itemize}


\section{Unions}

Have you ever thought about having a variable that could hold any data type of your choice? If so, C has your back by providing you with unions. Unions are pretty much like structs in their definition – However, their memory layouts are completely different. Meanwhile the computer lay out struct fields one after the other in memory, a union only has space for its largest field in memory. The computer will always set the size of the union to the largest field to make sure that the union is always large enough to hold any field inside it.
Assume you have the following union called quantity, with fields count, volume and total defined as:
...
Your ‘quantity’ variable will be able to support 3 different data types i.e short, float and int. You can only store 1 value in a union at a time! Whether you set the count, volume or total, the data will always go into the same memory space. In this case, the size of our union will be sizeof float, since float is the biggest datatype in this union. 
Any of the following 2 styles can be used to declare union variables:

\begin{verbatim}
quantity q = { .volume=6.4};    // Designated Initializers

quantity q; 
q.count = 4;                    // Dot notation
OR
quantity q = {4};               // C89 style (limited) - Use only if 
                                //you want to set the 1st field.

\end{verbatim}


\section{Enums}
An enum is another user defined type in C just like structs and unions. An enum variable stores a list of symbols. These symbols assign names to integral constants which makes your program easier to read and maintain. They are defined as follows:

\begin{verbatim}    enum colors = { BLUE, BLACK, RED }; \end{verbatim}

Every enum colors variable can only be assigned a symbol from the colors list.

\begin{verbatim}    enum colors favorite = BLUE; \end{verbatim}

Underneath the hood, the computer assigns constant integers to the different symbols listed in the enum, counting from 0 at the first symbol, when the symbols are undefined by the user as in the above example.

You may also set the integers to any of the symbols in your enum manually:

\begin{verbatim}
    enum week{MON, TUE, WED, THUR = 10, FRI, SAT=16, SUN};
    
\end{verbatim}

In this case, the computer sets the numeric value of an undefined symbol by counting from the value of the previously defined symbol. Ex; FRI will be assigned 11 by the computer, while SUN will be assigned 17.
Example:
...
enum day{Mond, Tues, Wedn, Thurs, Frid, Satu, Sund};
int main() {
